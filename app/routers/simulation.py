...
def create_project_from_simulation(simulation_id: str, name: str, db: Session = Depends(get_db), user_id: str = Depends(get_current_user_id)):
...
def get_simulation(simulation_id: str, db: Session = Depends(get_db), user_id: str = Depends(get_current_user_id)):
    simulation = db.query(Simulation).filter(Simulation.id == simulation_id).first()
    if not simulation:
        raise HTTPException(status_code=404, detail="시뮬레이션을 찾을 수 없습니다")
